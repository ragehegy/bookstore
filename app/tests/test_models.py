...
class BookModelTest(TestCase):
    ...
    def setUpTestData(cls):
        Book.objects.create(
            title="Test Book 1", author="Author 1", date_published="2024-10-16"
        )
...
class ReviewModelTest(TestCase):
    ...
    def setUpTestData(cls):
        book = Book.objects.first()
        user = User.objects.first()
        Review.objects.create(user=user, book=book, content="test good review")
        Review.objects.create(user=user, book=book, content="test bad review")
